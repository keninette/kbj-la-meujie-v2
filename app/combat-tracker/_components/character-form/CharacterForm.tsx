...
const CharacterForm = ({ initialCharacter, onSubmit }: CharacterFormProps) => {
  const [character, setCharacter] = useState<CombatCharacter|null>(initialCharacter);
  ...
  const handleSubmit = () => {
    ...
    console.log("Personnage ajouté :", character);
    ...
    <div className={styles["character-form"]}>
      ...
      <form onSubmit={handleSubmit}>
        <fieldset className={styles["character-form__fieldset"]}>
          ...
          <input type="text" id="name" name="name" value={character?.name || ""} onChange={handleInputChange} />
          </div>
          <div className={styles["character-form__line"]}>
          <label htmlFor="hp">PV</label>
          <input type="number" id="hp" name="hp" value={character?.hp ?? 0} onChange={handleInputChange} />
          </div>
          <div className={styles["character-form__line"]}>
          <label htmlFor="maxHp">PV max</label>
          <input type="number" id="maxHp" name="maxHp" min="1" value={character?.maxHp ?? 0} onChange={handleInputChange} />
          </div>
          <div className={styles["character-form__line"]}>
          <label htmlFor="ac">CA</label>
          <input type="number" id="ac" name="ac" value={character?.ac ?? 0} onChange={handleInputChange} />
          </div>
          <div className={styles["character-form__line"]}>
          <label htmlFor="initiative">Initiative</label>
          <input type="number" id="initiativeScore" name="initiativeScore" value={character?.initiativeScore ?? 0} onChange={handleInputChange} />
          </div>
        </fieldset>
        <fieldset className={styles["character-form__fieldset"]}>
        ...
        <fieldset className={styles["character-form__fieldset"]}>
          ...
          <ButtonWithIcon label="Enregistrer et recommencer" onClick={() => console.log("Personnage enregistré")} faIcon="refresh" faIconStyle={FaIconStyleEnum.SOLID} iconPosition="left"/>
        </fieldset>
      </form>
    </div>
  );
